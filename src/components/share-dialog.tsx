...
export function ShareDialog({ carId, carName, trigger }: { carId: string; carName: string; trigger?: ReactNode }) {
  ...
  const invite = useMutation({
    mutationFn: async () => {
      ...
      if (!lookup) throw new Error("No Apex Setup user with that email");
      ...
      <DialogContent className="max-w-lg">
        ...
        <div className="space-y-3">
          ...
          <p className="text-xs text-muted-foreground">
            They must already have an Apex Setup account. Editors can change setups & log laps; viewers are read-only.
          </p>
        </div>
